...
struc = postProcessPathASTRA_dsm_lowThrust( dv, output, MAT, ...
    vdep_free, varr_free, ...
...
[rrpl2, vvpl2]        = EphSS_from_mice_workaround( struc(1).idA, t2, idcentral );
vinf_mars = norm( vvpl2 - state2(:,4:6) )

[mu_planet, radius]      = planetConstants(struc(1).idA);
flyby_alt  = minsol(end) * radius - radius
dvD
...
% param.epsilon       = 0.002;
% param.initial_guess = LTsol.lambdas;
% param.rho           = 1; 
% param.rhoLim        = 0.001;
% param.gamma         = 0.9; 
% 
...
if need_to_refine
    ...
    for ind = 1:62
        ...
        fprintf( "RHO: %f \n", param.rho );
        [initiallamba, Fsol] = ...
                    fsolve(@(lambda0) propagateState_SEP(lambda0, @propagateFopt_SEP_MEXIFY_mex, param), ...
        ...
        transfer_1 = postProcessLT_SEP( time, states, @propagateFopt_SEP_MEXIFY_mex, param );
...
struc(2).tA = t2;
struc(2).xxAtar = state2;
write_mga_param( struc, INPUT, '/EMC_2045_4798y_a64_monoboost.txt' );
...
Tmax_temp                   = 0.35;
param_temp                  = processDataAndWriteParam(m0_new, tof_sec, ...
                                state1, state2, ...
                                Tmax_temp, Isp, g0, ...
                                Nrev, idcentral, useParallel);
param_temp.gamma            = 0.5;
param_temp.rhoGuess1        = 0.5;
param_temp.rhoLim           = 0.001;
param_temp.use_energy_guess = true;
...
for ind = 1:70
    ...
    transfer_2 = postProcessLT_SEP( time, states, @propagateFopt_SEP_MEXIFY_mex, param );
...
% Place the labels
text(struc(1).xxDtar(:,1)./AU,  struc(1).xxDtar(:,2)./AU + 0.1, 'Earth', ...
    'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', ...
    'FontSize', 20, 'FontWeight', 'bold');

text(state1(:,1)./AU,  state1(:,2)./AU + 0.1, 'Mars', ...
    'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', ...
    'FontSize', 20, 'FontWeight', 'bold');
...
% 
% % save_fig_custom( figTRAJ, custom_dpi, 'svg', 'figTRAJ.svg' );
% % save_fig_custom( figMASS, custom_dpi, 'svg', 'figMASS.svg' );
% % save_fig_custom( figTHRmag, custom_dpi, 'svg', 'figTHRmag.svg' );
% 
% % save_fig_custom( figTRAJ, custom_dpi, 'pdf', 'figTRAJ.pdf' );
% % save_fig_custom( figMASS, custom_dpi, 'pdf', 'figMASS.pdf' );
% % save_fig_custom( figTHRmag, custom_dpi, 'pdf', 'figTHRmag.pdf' );
