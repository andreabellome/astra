function [OUTPUT] = ASTRA_SODP_v2(INPUT, seq)

% -------------------------------------------------------------------------
%
% MGA SINGLE-OBJECTIVE (SO) OPTIMIZATION WITH DYNAMIC PROGRAMMING (DP)
%
% -------------------------------------------------------------------------
...
if ~isempty(MSSTRUC(1).STRUC)
    ...
    for indms = 2:length(MSSTRUC)
        ...
        for indmm = 1:size(opt,1)
            ...
            tocTOT = tocTOT + toc(tic0); toc(tic0)
...
if ~isempty(MSFINAL)
    ...
    for inds = 1:length(STRUC)
        ...
        OUTPUT(inds) = saveOUTPUT_SODP(LEGSn, VASn, VINFn, INPUT, runOpts, chRevs(inds,:), tocTOT);
...
try
    if INPUT.plot(2) == 1
        ...
        plot( PF(:,1), PF(:,2), 'o', 'MarkerEdgeColor', 'Black', 'MarkerFaceColor', 'Green' );
    end
catch
end
